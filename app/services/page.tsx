          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {company.services.map((s, i) => (
              <Reveal key={s.title} delay={(i % 3) * 60}>
                <div className="flex h-full flex-col bg-surface shadow-card hover:shadow-card-md transition-shadow rounded-sm p-7">
                  <span className="font-mono text-[12px] tracking-[0.16em] text-amber">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <h3 className="display-tight mt-4 text-xl text-ink">{s.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-ink-soft flex-1">{s.body}</p>
                </div>
              </Reveal>
            ))}
          </div>

          <p className="mt-12 max-w-3xl text-[15px] leading-relaxed text-ink-soft border-t border-line pt-10">
            Whether it is a government tender, municipal sanitation project, institutional
            requirement, or private development, we provide end-to-end support from concept to
            deployment.
          </p>
        </div>
      </section>
